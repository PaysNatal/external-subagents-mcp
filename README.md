...
A read-only MCP server that lets Codex delegate bounded workspace exploration, large-context review, summarization, file discovery, and log analysis to external OpenAI-compatible models.
...
Each profile assigns five task roles to the providers above. The roles are: `explorer`, `summarizer`, `reviewer`, `log_analyst`, and `file_finder`.

| Profile | Explorer | Summarizer | Reviewer | Log analyst | File finder | Best for |
|---------|----------|------------|----------|-------------|-------------|----------|
| `single_provider` | standard | standard | standard | standard | standard | Initial setup — one model does everything |
| `cost_first` | lite | lite | pro | lite | lite | Save cost — only review gets the strong model |
| `quality_first` | pro | lite | pro | pro | pro | Maximize quality — exploration and analysis use the strong model |
...
Existing configs without an explicit `explorer` role remain valid. The server derives it from `file_finder`, then `summarizer`, then the first configured role.
...
Codex should prefer sending file paths instead of copying large source or log bodies into a tool call. If the project is not the MCP server's default workspace, all five task tools accept:
...
| Tool | Purpose |
|------|---------|
| `delegate_explore_workspace` | Iteratively list, search, and read an unfamiliar authorized workspace |
...
Use `delegate_explore_workspace` when Codex has a concrete question but does not yet know which files contain the answer. It requires an OpenAI-compatible provider with tool-calling support and exposes only bounded `list_files`, `search_text`, `read_file`, and `read_file_range` operations. Defaults are 8 turns, 40 distinct files, and 1 MiB of source; hard caps are 20 turns, 200 files, and 5 MiB. If the provider cannot call tools, use the known-path tools or let Codex investigate directly.

When paths are already known, prefer `delegate_summarize_paths`, `delegate_review_diff`, `delegate_find_relevant_files`, or `delegate_analyze_log`. They use fewer provider turns and make the delegated labor easier for Codex to verify.
...
- `inputBytes`: UTF-8 bytes in the initial task prompt. For explorer jobs, use `exploration.sourceBytesRead` to see how much workspace source the tool loop read.
...
- `exploration`: explorer turns, tool calls, files read, source bytes read, search matches returned, and any limits reached.
...
The server reports external provider usage when available, but does not claim exact main-model token savings because Codex does not expose reliable per-tool main-model token accounting.
...
- Workspace exploration is bounded and read-only; external delegates never receive edit, shell, test, or implementation tools.
...
一个只读 MCP 服务器，让 Codex 将有界 workspace 探索、大上下文代码审查、摘要、文件定位和日志分析委托给外部 OpenAI-compatible 模型。
...
每个 profile 将五种任务角色分配给上述 provider。角色为：`explorer`、`summarizer`、`reviewer`、`log_analyst`、`file_finder`。

| Profile | Explorer | Summarizer | Reviewer | Log analyst | File finder | 适用场景 |
|---------|----------|------------|----------|-------------|-------------|----------|
| `single_provider` | standard | standard | standard | standard | standard | 初始配置 — 一个模型处理所有任务 |
| `cost_first` | lite | lite | pro | lite | lite | 省成本 — 只有代码审查用强模型 |
| `quality_first` | pro | lite | pro | pro | pro | 重质量 — 探索和分析使用强模型 |
...
Codex 应优先传递文件路径，而不是把大段源码或日志复制进工具调用。当目标项目不是 MCP 服务器的默认 workspace 时，五个任务工具都可传入：
...
| 工具 | 用途 |
|------|------|
| `delegate_explore_workspace` | 在陌生的已授权 workspace 中迭代列出、搜索和读取文件 |
...
当 Codex 有明确问题、但还不知道答案位于哪些文件时，使用 `delegate_explore_workspace`。它要求 OpenAI-compatible provider 支持 tool calling，并且只开放有界的 `list_files`、`search_text`、`read_file`、`read_file_range`。默认上限为 8 轮、40 个不同文件和 1 MiB 源码；硬上限为 20 轮、200 个文件和 5 MiB。provider 不支持 tool calling 时，应改用已知路径工具，或由 Codex 自行调查。

路径已知时，优先使用 `delegate_summarize_paths`、`delegate_review_diff`、`delegate_find_relevant_files` 或 `delegate_analyze_log`。它们需要更少的 provider 轮次，也更方便 Codex 复核委托结果。
...
- `inputBytes`：初始任务 prompt 的 UTF-8 字节数。Explorer job 在工具循环中读取的 workspace 源码规模请查看 `exploration.sourceBytesRead`。
...
- `exploration`：explorer 使用的轮次、工具调用数、读取文件数、源码字节数、返回的搜索匹配数和触发的限制。

缓存命中时 `externalApiCalled` 为 `false`；此时附带的 usage 表示原始缓存任务的历史消耗，不代表本次产生了新调用。provider 不返回 usage 时仍可正常使用，服务器不会伪造精确 token 数。

服务器会尽量记录外部 provider usage，但不会声称精确节省了多少主模型 token，因为 Codex 不提供可靠的单工具主模型 token 统计。
...
- Workspace 探索始终有界且只读；外部代理不会获得编辑、shell、测试或实现工具。
